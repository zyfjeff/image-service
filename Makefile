...
docker-smoke:
	...
	docker run --rm --privileged -v $(TEST_WORKDIR_PREFIX) -e TEST_WORKDIR_PREFIX=$(TEST_WORKDIR_PREFIX) -v ${current_dir}:/nydus-rs -v ~/.ssh/id_rsa:/root/.ssh/id_rsa -v ~/.cargo:/usr/local/cargo -v fuse-targets:/nydus-rs/target-fusedev -v virtiofs-targets:/nydus-rs/target-virtiofs nydus-rs-smoke
